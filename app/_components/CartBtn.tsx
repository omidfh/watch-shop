...
import { getCartItemNumber } from "../_lib/data-service";

export default async function CartBtn({ userId }: { userId: number }) {
  const cartItems = await getCartItemNumber(userId);
  return (
    <div>
      <Link
        href={"/profile/cart"}
